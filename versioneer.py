...
def render_pep440_ricequant(pieces):
    ...
    if tracking_branch in ["origin/develop", "origin/master"] or tracking_branch.startswith("origin/hotfix/"):
        return rendered
    rendered += ".g%s" % pieces["short"]
    return rendered
    ...
    for r in parse_requirements("./requirements.txt", session=PipSession()):
        for spec in r.specifier._specs:
            ...
            if not is_canonical(spec.version):
                continue
            if re.match(r'^([1-9][0-9]*!)?(0|[1-9][0-9]*)(\.(0|[1-9][0-9]*))*(\.post(0|[1-9][0-9]*))?$',
